...
import {isRegisterLoading} from "../../features/auth/selectors";
import styles from './styles';

export default function Register({navigation}: AuthStackScreenProps<'Register'>) {
    const dispatch = useDispatch();
    const [username, setUserName] = useState<string>('');
    const [password, setPassword] = useState<string>('');
    const isLoading = useSelector(isRegisterLoading)

    const onLoginProceedPress = useCallback(() => {
    ...
    return (
        <View style={styles.container}>
            ...
            <Spacer height={40}/>
            <Button loading={isLoading} onPress={onLoginProceedPress} disabled={isDisabled}>
                Register
            </Button>
            <Spacer height={18}/>
